...
PATH = os.path.join(os.path.dirname('./2'), './2')
...
for x in image_files:
  ...
  train_data.append(load_image('./2/'+x))
...
for x in range(20000):
    ...
    line = f.readline()
f.close()
train_label=tf.reshape(train_label,[10000])
...
BATCH_SIZE = 10
train_image_count = 10000#train_data.shape[0]
...
generator.load_weights('./generatorlosscheckpoint')
...
def discriminator_loss(real_cat_out, label): # 接收真图 和 真实图片的分类  假图 加label
    
    cat_loss = category_cross_entropy(label, real_cat_out)
    return cat_loss
def generator_loss(fake_cat_out, label):
...
def train_step(images, labels,gloss,dloss):
    ...
    with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
        ...
        #real_cat_out = discriminator(images, training=True)
        fake_cat_out = discriminator(generated_images, training=True)
        ...
        gen_loss1 = generator_loss(fake_cat_out, labels)
        gen_loss = gen_loss1 + 10*dif_loss
        #gen_loss = gen_loss1
        #disc_loss = discriminator_loss(real_cat_out, labels)
    gloss=gen_loss1
    dloss=dif_loss  
    #gloss = 0
    #dloss = disc_loss 
...
EPOCHS = 50
t=train_label.numpy()
print(t[0],t[1],t[2],t[3],t[4],t[5],t[6],t[7],t[8],t[9],t[10],t[11],t[12],t[13],t[14],t[15],t[16],t[17],t[18],t[19])
